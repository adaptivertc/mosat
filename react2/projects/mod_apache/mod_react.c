...
static int react_handler(request_rec* r) 
{
  pthread_t self;
  pid_t lwp;

  if ( (r->handler == NULL) || (0 != strcmp(r->handler, "react")) ) 
  {
 ...
 ****/ 
  ...
  ap_rputs(
	"<html><head><title>Apache REACT Module</title></head>", r) ;
  ap_rputs("<body><h1>Hello REACT Users!</h1>", r) ;
  ap_rputs("<p>This is the Apache REACT module!</p>", r) ;
...
static void react_init_handler(apr_pool_t *pchild, server_rec *s) 
{
  init_hook_n_calls++;
...
static void react_hooks(apr_pool_t* pool) 
{
  ap_hook_handler(react_handler, NULL, NULL, APR_HOOK_MIDDLE) ;
  ap_hook_child_init(react_init_handler, NULL, NULL, APR_HOOK_MIDDLE);
}

/*****************************************************/

module AP_MODULE_DECLARE_DATA react_module = 
{
