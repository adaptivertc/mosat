...
void rgauge_t::generate(FILE *svg_fp, FILE *svg_top_of_file_fp, FILE *js_fp, int argc, char **argv)
{
  ...
  fprintf(svg_fp, "<!--  START insert for rgauge (%03d) -->\n", n_instance);
  fprintf(js_fp, "// --  START insert for rgauge (%03d)\n", n_instance);

  char js_object_name[30];
  snprintf(js_object_name, sizeof(js_object_name), "rgauge_obj_%03d", n_instance);
  fprintf(js_fp, "var %s = new rgauge_t(\"rgauge_pv_%03d\", "
  ...
  fprintf(svg_fp, "<circle cx=\"%lf\" cy=\"%lf\" r=\"%lf\" fill=\"antiquewhite\" stroke=\"black\" stroke-width=\"%lf\" filter=\"url(#filter1)\"/>\n", 
         cx, cy, r, 2.0 * scale_factor);

  fprintf(svg_fp, "<text id=\"rgauge_pv_%03d\" text-anchor=\"middle\" x=\"%lf\" y=\"%lf\" font-size=\"%lf\" fill=\"%s\" filter=\"url(#filter1)\">00</text>\n",
         n_instance, cx - (5 * scale_factor), cy + (15 * scale_factor), 50.0 * scale_factor, rcolor); 

  fprintf(svg_fp, "<line x1=\"%lf\" y1=\"%lf\" x2=\"%lf\" y2=\"%lf\" stroke =\"darkgray\" stroke-width=\"%lf\"/>\n", 
  ...
  fprintf(svg_fp, "<text id=\"rgauge_eu_%03d\" text-anchor=\"middle\" x=\"%lf\" y=\"%lf\" font-size=\"%lf\" fill=\"%s\" filter=\"url(#filter1)\">amps</text>\n",
                n_instance, cx, cy + (33.0 * scale_factor), 15 * scale_factor, rcolor); 

  fprintf(svg_fp, "<text id=\"rgauge_hi_%03d\" text-anchor=\"start\" x=\"%lf\" y=\"%lf\" font-size=\"%lf\" fill=\"%s\" filter=\"url(#filter1)\">0</text>\n", 
       n_instance, cx - (23.0 * scale_factor), cy + (50.0 * scale_factor), 8 * scale_factor, rcolor); 

  fprintf(svg_fp, "<text id=\"rgauge_lo_%03d\" text-anchor=\"end\" x=\"%lf\" y=\"%lf\" font-size=\"%lf\" fill=\"%s\" filter=\"url(#filter1)\">30</text>\n", 
  ...
  fprintf(svg_fp, "<!--  END insert for rgauge (%03d) -->\n", n_instance);
  fprintf(js_fp, "// --  END insert for rgauge (%03d)\n", n_instance);
