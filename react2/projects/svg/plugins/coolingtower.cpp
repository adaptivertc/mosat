...
void cooling_tower_t::generate(plugin_data_t d, int argc, char **argv)
{
  ...
  fprintf(d.svg_fp, "<polygon points=\"%lf,%lf %lf,%lf %lf,%lf %lf,%lf\" stroke=\"black\" stroke-width=\"%lf\" fill=\"%s\"/>\n",
             x1, y1, x1 + width, y1, x1 + (40 * scale_factor), y2, x1 + (10.0 * scale_factor), y2, 0.5 * scale_factor, color);
  fprintf(d.svg_fp, "<text x=\"%lf\" y=\"%lf\" font-size=\"%lf\" fill=\"%s\" text-anchor=\"middle\">%s</text>\n",
