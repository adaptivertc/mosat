...
eeprom int error;
int current_index;
unsigned long actual_current;
unsigned long raw_current;
unsigned long raw_currents;
//int do_channel;
int tick;
int buzz_top;
int buzzers[MAXBUZZ];
...
void set_current_index()
{
        ...
        low_limit = current_limits[current_index] - 521;
        low_normal_limit = current_limits[current_index] - 348;
        normal_limit = current_limits[current_index];
        high_normal_limit = current_limits[current_index] + 173;
        high_limit = current_limits[current_index] + 346;
        very_high_limit = current_limits[current_index] + 434;
...
int i;
    ...
    current_limits[0] = 2221;
    current_limits[1] = 2251;
    current_limits[2] = 2281;
    current_limits[3] = 2311;
    
    
/*100amp en un hilo son 511 en el adc y 1730 en 3 hilos

110amp en un hilo son 562 en el adc y 1903 en 3 hilos

130amp en un hilo son 665 en el adc y 2251 en 3 hilos

140amp en un hilo son 716 en el adc y 2424 en 3 hilos

150amp en un hilo son 767 en el adc y 2597 en 3 hilos

155amp en un hilo son 793 en el adc y 2685 en 3 hilos*/
...
int i;
        ...
        for(i = 0; i < 650; i++)
                raw_currents += adc_data[0];
        raw_current = raw_currents/650;
        
        //raw_current = adc_data[0];
        
        actual_current = ((unsigned long)FACTOR_AMPERS*(unsigned long)raw_current)/(unsigned long)1023;
}
...
void info()
{
        printf("Valor acumulado ADC: %lu\n\r",raw_currents);
        printf("Valor ADC: %lu\n\r",raw_current);
        printf("Corriente 1 hilo: %lu\n\r",((long)200*(long)raw_current)/(long)1023);
        printf("Corriente 3 hilos: %lu\n\r",actual_current);
        printf("Corriente promedio: %d\n\r",current_limits[current_index]);
        printf("Low Limit: %d\n\r",low_limit);
        printf("Low Normal Limit: %d\n\r",low_normal_limit);
        printf("Normal Limit: %d\n\r",normal_limit);
        printf("High Normal Limit: %d\n\r",high_normal_limit);
        printf("High Limit: %d\n\r",high_limit);
        printf("Very High Limit: %d\n\r",very_high_limit);        
}
...
inicializar();
letra = '#';
first_state = second_state = low_limit;
...
{
        if(second_state == low_limit && (actual_current < low_normal_limit && actual_current > low_normal_limit-20))
        {
                write_do(0,'0');
        }
        else
...
{
        if(second_state == low_normal_limit && (actual_current < normal_limit && actual_current > normal_limit -20))
        {
                write_do(1,'0');
        }
        else
