...
onboard_config_t::onboard_config_t(void)
{
  n_keys = 0;
  cur_key = 0;
...
double onboard_config_t::get_double(const char *aKey)
{
  for (int i=0; i < n_keys; i++)
  {
    if (0 == strcasecmp(aKey, key[i]))
...
const char *onboard_config_t::get_config(const char *aKey)
{
  for (int i=0; i < n_keys; i++)
  {
    if (0 == strcasecmp(aKey, key[i]))
...
void onboard_config_t::read_file(const char *path)
{
  char fname[100];
  if (path == NULL)
  {
